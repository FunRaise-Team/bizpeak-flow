...
### Phase 3.13: POC v6（2026-07-03 晚、訪談前最終輪）
- [x] 報價整合：quote_create 內建 Co-Evo 三方案（拆自 funraise-co-evo-quotation 外掛、v4 格式/單號/稅則/17 條款全保留）、三入口同步、「報價」頁籤（15 筆真報價、漏斗、編輯連結、一鍵建約）— 業務免各自分支
- [x] 樣本重造：11 合約全狀態（含超時 ×2、逾期 D+38、續約窗口 ×1、作廢）、款項 5-11 月散佈、歷史 11、留言 3（含經助理範例）
- [x] 白話化：SLA→處理時限、留痕→操作紀錄（收合）、事件頁籤→紀錄
- [x] 驗收：報價實建一單（單號流水正確、測後封存）、本地與線上煙測全過、Notion 提案 v0.7

### Phase 3.14: POC v7（2026-07-03 夜、訪談前封版）
- [x] 中文組字保護（選字 Enter 不誤送）；聊天輕排版
- [x] 人員屬性：負責人 people 欄（users API 33 位比對、樣本回填）；留言 @姓名 真提及（Notion 通知、實測 True）
- [x] 各期金額不均分（逗號/百分比、驗總和、C4 照拆、逐期開票）— 實測 20/30/50% 精確
- [x] demo_lifecycle 演示模式（11 步含守門、自動清理 19 筆）；ui_tour 六步導覽（線上驗證觸發）
- [x] 部署＋線上煙測；封版待訪談回饋

### Phase 3.15: POC v8（2026-07-03 傍晚、訪談前真封版）
- [x] 聊天權限補齊：product_create／product_update／quote_detail／template_list 四工具宣告進聊天（根因：能力在但從未宣告給 Gemini）— 聊天實測「改產品說明」「給我 NDA 條款」全過、助理自述權限正確
- [x] 全盤關聯：Contracts 加「客戶連結」跨庫 relation（→ 客戶大名單 509 家）；contract_create 自動掛客戶、quote_import 依案型自動掛產品（CASE_TO_PRODUCT）＋順手修真 bug：報價列原本沒讀「案型」欄、關聯永遠落空
- [x] 樣板庫：Templates DB 新建＋種 5 樣板（Co-Evo 17 條款、通用 B2B 條款、NDA 雙向版、催收信三階段、教育訓練條款；法務未審者標「草稿待法務審」）；產品頁籤下渲染、聊天/MCP 可取用
- [x] 導覽 v2：7 步（補產品與樣板庫）、4.5 秒/步、每步頁籤＋面板高亮、說明泡泡留聊天框、點泡泡跳回該步畫面
- [x] mdLite 升級（標題/行內反引號/表格線降級）＋系統提示詞加「回覆用純文字排版」— markdown 原始格式雙管根治
- [x] MCP 入口補齊 v8 四工具；樣本客戶回填檢查（11 家全虛構、0 命中屬預期、真實客戶實測 relation 寫入成功）
- [x] 驗證：後端 10/10（含真客戶建約→驗 relation→清除）、前端 headless Firefox 9/9（導覽/高亮/回點/mdLite/樣板渲染、零頁面錯誤）
- [x] 獨立驗收（fresh-context 代理、線上 8 項、三輪往返）：首輪 7/8 抓到「催收信範本」線上空回覆（Gemini 第一輪就被擋、本地不觸發、產製者自測沒抓到 → 驗證不自驗的實證）；二輪複驗又抓到備援只接第二輪空；最終三管齊下（safetySettings BLOCK_NONE＋工具結果備援 _fmt_fallback＋第一輪空原句重打 ×2、69985b0＋5fe1c62）— 我方壓力測 8/8、代理終驗 3/3 收案
