...
TOOL_DECLS = [
    ...
    {"name": "ui_navigate", "description": "切換使用者眼前的介面頁籤、可高亮某編號。回答涉及某頁籤資料時呼叫、讓使用者直接看到",
     "parameters": {"type": "object", "properties": {
         "tab": {"type": "string", "enum": ["overview", "payments", "overdue", "events"]},
         "highlight": {"type": "string", "description": "要高亮的合約或款項編號（可選）"}}, "required": ["tab"]}},
]

SYS = """你是 BizPeak Flow（FUNRAISE 合約生命週期系統）介面裡的操作助理、使用者正在看網頁介面（頁籤：overview 合約總覽 / payments 收款 / overdue 逾期 / events 事件）。
規則：
- 查詢與操作一律用工具、絕不編造資料
...
WRITE_FNS = {"contract_transition", "payment_mark_paid"}
